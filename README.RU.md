...
Проект НЕ ЗАВЕРШЕН, а именно:
- не написаны скрипты для off-chain вызова методов контрактов
- не написаны все тесты для CPoolGames
- не полностью оттестирован
- очевидно, есть неоптимальности, потому что Tezos и PascaLIGO я увидел меясц назад впервые  
...
Папки html-en и html-ru и их содержимое сгенерированы автоматически.  

Презентация о проекте в текстовых файлах slides-ru/*.slide (все остальные файлы в папке сгенерированы скриптами), скрипт `slides.sh` генерирует видеопрезентацию slides-ru/video.mp4.
...
- Гибкое конфигурирование кода (см. contracts/CPoolGames/config.ligo):
    - владелец/администраторы пула или пул-как-услуга
...
- Случайное число основано на хеше ближайшего блока Tezos с бОльшим временем запроса и XOR с хэшем запроса, поэтому один блок Tezos генерирует разные случайные числа для разных запросов с одинаковым временем.
...
Английские тексты для слайдов в *.slide и другие автогенерированные slides.sh файлы   

#### doc/slides-ru

Русские тексты для слайдов в *.slide и другие автогенерированные slides.sh файлы   
